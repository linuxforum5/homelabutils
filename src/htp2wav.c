/***
 * Homelab HTP fájlok wav fájlra konvertálása
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
int sync_us = 150; // 48000Hz esetén 50-200-ig ok Homelab 4 alapján 200us
int bit_length = 1600; // in us
int prefix_silence_length = 2000;
int turbo_divider = 1; // Ennyivel osztja a turbósítható bitidőket turbó módban
...
void write_peaks( FILE *wav, int us, unsigned char value, int turbo ) {
    int sample_length_us = 1000000 / waveHeader.nSamplesPerSec;
    int cnt = us / sample_length_us;
    if ( turbo ) cnt /= turbo_divider;
    for( int i = 0; i < cnt; i++ ) fputc( value, wav );
}

void write_bit_into_wav( FILE *wav, unsigned char bit, int last ) { // Az utolsó bitet hagyományos véggel írjuk ki turbó módban is
    int space_us = bit_length - sync_us;
    unsigned char nulla = low; // NEG_PEAK
    if ( bit ) {
        write_peaks( wav, ( space_us - sync_us ) / 2, nulla, 1 );
        write_peaks( wav, sync_us, high, 1 );
        write_peaks( wav, ( space_us - sync_us ) / 2, nulla, 1 );
    } else {
        write_peaks( wav, space_us, nulla, 1 );
    }
    write_peaks( wav, sync_us, high, 1 ); //!last );
}
...
void write_byte_into_wav( FILE* wav, unsigned char c ) { // Bitkiírási sorrend: először a 7. bit legvégül a 0. bit
    for( int i = 0; i < 8; i++ ) {
        ...
        write_bit_into_wav( wav, bit, i==7 );
    }
}
...
static void init_wav( FILE *wavfile ) {
    ...
    /* Lead in silence */
    write_peaks( wavfile, prefix_silence_length, silence, 0 );
}
...
//    write_byte_into_wav( output, 0 );
    write_peaks( output, 2000, silence, 0 );
}    

static void print_usage() {
    printf( "Htp2Wav v1.0\n");
    printf( "Homelab 2/3 Htp to Wav v1.1\n");
    ...
    printf( "-T <div>   : Div bit peak lengths\n");
...
int main(int argc, char *argv[]) {
    ...
    while (!finished) {
        switch (getopt (argc, argv, "?hf:H:S:p:L:P:B:T:i:o:g:")) {
            case -1:
            case ':':
                finished = 1;
                break;
            ...
            case 'B':
                ...
                break;
            case 'T':   // Turbo divider value
                if ( !sscanf( optarg, "%i", &arg1 ) ) {
                    fprintf( stderr, "Error parsing argument for '-T'.\n");
                    exit(2);
                } else {
                    turbo_divider = arg1;
                }
