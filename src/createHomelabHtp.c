...
void create_bin_htp_block( FILE *htp, FILE *bin, uint16_t bin_load_address, const char* htp_name, int is_last ) {
    if ( verbose ) printf( "Create htp block, named '%s' load address: %04X\n", htp_name, bin_load_address );
...
int main(int argc, char *argv[]) {
    ...
    if ( htp ) {
        ...
        if ( txt ) create_bas_htp_block( htp, txt, htp_name, !bin, BAS );
        if ( BAS ) fclose( BAS );
        if ( bin ) create_bin_htp_block( htp, bin, bin_load_address, txt ? 0 : htp_name, !bin2 );
